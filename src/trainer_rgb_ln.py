...
def train(model, tr_loader, vd_loader, beta, lr, epochs=5, prev_epoch=None, prev_i=None, summary=None, slide=50,
    ...
    torch.save(model.state_dict(), os.path.join(MY_FOLDER, f'models/final_run_{experiment}.pt'))
...
if __name__ == '__main__':
    ...
    if args.from_checkpoint:
        # Load checkpoint
        checkpoint = torch.load(os.path.join(MY_FOLDER, f'models/checkpoint_run_120.pt'),
                                map_location='cpu')
        model = nn.DataParallel(model)
