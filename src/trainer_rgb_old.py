...
def save_checkpoint(state, is_best, filename=os.path.join(f'{MY_FOLDER}', 'models/checkpoint.pt')):
...
def train(model, tr_loader, vd_loader, beta, lr, epochs=5, prev_epoch=None, prev_i=None, summary=None, slide=50,
        ...
        for i, data in enumerate(tr_loader):
            ...
            if (i % 50 == 0) and (i != 0):
                ...
                save_checkpoint({
                    ...
                    filename=os.path.join(MY_FOLDER, f'models/checkpoint_run_{experiment}.pt'))
    ...
    torch.save(model.state_dict(), ofilename=os.path.join(MY_FOLDER, f'models/checkpoint_run_{experiment}.pt'))
...
if __name__ == '__main__':
    ...
    if args.from_checkpoint:
        # Load checkpoint
        checkpoint = torch.load(os.path.join(os.environ.get('OUT_PATH'), f'models/checkpoint_run_{args.experiment}.pt'),
                                map_location='cpu')
        model = nn.DataParallel(model)
