...
AUDIO_FOLDER = '/mnt/gpid07/imatge/teresa.domenech/venv/PixInWav/PixInWav_dataset'
...
class AudioProcessor():
    ...
    def __init__(self, transform, audio_path):
        ...
        self._limit = len(torchaudio.load(audio_path))#67522 # 2 ** 16 + 2 ** 11 - 2 ** 6 + 2
       # self._frame_length = 2 ** 12 if transform == 'cosine' else 2 ** 11 - 1
        self._frame_length = len(torchaudio.load(audio_path))
        print('len audio',self._frame_length)
...
class StegoDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self._image_data_path = pathlib.Path(image_root) / 'train2017'
        #self._audio_data_path = pathlib.Path(f'{audio_root}{folder}')
        self._audio_data_path = pathlib.Path(audio_root) / 'train'
        self._MAX_LIMIT = 7983 if folder == 'train2017' else 900
        self._MAX_AUDIO_LIMIT = 7983 if folder == 'train' else 7983
        ...
        for key in mappings.keys():
            ...
            if self._index == self._MAX_LIMIT:
                break
        """""
        #IMAGES SHA DE MIRAR A TRAIN VAL I TEST
        _auxi_index = 0
        for img in glob.glob(f'{self._image_data_path}/*.{self.image_extension}'):
            ...
            if _auxi_index == 900:
                break
        _auxiv_index = 0
        for img in glob.glob(f'/mnt/gpid08/datasets/coco-2017/coco/images/val2017/*.{self.image_extension}'):
            self._images_val.append(img)
            _auxiv_index += 1
            if _auxiv_index == 7983:
                break
        ...
        self._AUDIO_PROCESSOR = AudioProcessor(transform=self._transform, audio_path=audio_path)
    ...
    def __getitem__(self, index):
        ...
        rand_indexer_img = random.randint(0, self._MAX_LIMIT - 1)
        #img_path = glob.glob(f'{self._image_data_path}/{key}/{key}_{indexer}.{self.image_extension}')[0]
        #audio_path = self._audios[rand_indexer]
        audio_path = self._audios[rand_indexer]
        audio_name = audio_path.split('/')[9]
        audio_id = audio_name.split('_')[2]
        #print(audio_id, 'audio_id')

        for imgg in self._images_val:
            find=False
            im = imgg.split('/')[8]
            im = im.split('.')[0]
            im = int(im)
        ...
        if find==False:
            img_path = self._images[rand_indexer_img]
            print('RANDOM')
...
def loader(set='train2017', rgb=True, transform='cosine'):
    """
